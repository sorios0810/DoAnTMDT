...
namespace DoAn_LapTrinhWeb.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            //hiển thị những đồng hồ được mua nhiều nhất
            List<Product> watchsales = db.Products.Where(item => item.status == "1" && item.quantity != "0" && item.Genre.ParentGenres.id == 2).Take(4).OrderByDescending(item => item.buyturn).ToList();
            ViewBag.WatchSales = watchsales;
            //hiển thị những trang sức được mua nhiều nhất
            List<Product> jewelsales = db.Products.Where(item => item.status == "1" && item.quantity != "0" && item.Genre.ParentGenres.id == 8).Take(4).OrderByDescending(item => item.buyturn).ToList();
            ViewBag.JewelSales = jewelsales;
            ...
            List<Brand> brand = db.Brands.Where(item => item.status == "1").Take(10).OrderByDescending(m => m.Products.Count()).ToList();
            ViewBag.Brand = brand;
            //banner khuyến mãi
